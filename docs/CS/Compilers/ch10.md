...
??? exmaple "Control Flow Graph 节点属性"
    ![Control Flow Graph 节点属性](../../assets/img/docs/CS/Compilers/ch10/image-6.png)
...
- 在每个 CFG 节点处建立局部方程（local equations）
    - 对于 CFG 节点 n，我们写
        - in[n]：在节点的所有入边上都为真的事实
        - out[n]：所有出边上的事实均为真
    - 定义将信息从一个节点传输到另一个节点的传输函数（transfer functions）
- 求解方程以计算所需信息
    - 迭代更新 in[n] 和 out[n]
...
- 定义（def）指对变量或临时变量的赋值
    - 一个变量的 def 指定义它的图节点集合
    - 一个图节点的 def 指它定义的变量集合
- 使用（use）指赋值右侧（或其他表达式中）出现了该变量
    - 一个变量的 use 指使用它的图节点集合
    - 一个图节点的 use 指它使用的变量集合
...
- Live on an edge：如果存在从该边（edge）到该变量 use，且不经过任何该变量 def 的一条有向路径，则该变量在该边上 live
    - 该变量稍后会被使用
    - 该变量在使用前不会被重新定义
- Live-in: 如果变量在该节点的任何入边上 live，则该变量在该节点上 live-in
- Live-out: 如果变量在该节点的任何出边上 live，则该变量在该节点上 live-out
- in[n]: 在节点 n 的 live-in 的集合
- out[n]: 在节点 n 的 live-out 的集合

此外，在这些定义的基础上定义活跃（liveness）分析的数据流方程

- 规则 1：如果 $a \in in[n]$，则对于 $\forall m \in pred[n]$，$a \in out[m]$
    - 如果一个变量是节点 n 的 live-in，则它在 n 的所有前驱节点上都是 live-out
...
在上述的例子中，我们可以感觉到迭代速度较慢，这是因为信息从一个节点传播到另一个节点的唯一方式是使用 $out[n] = \bigcup_{s \in succ[n]} in[s]$，在这种**向后传播**的特性下，如果使用顺控制流的方向，迭代速度会较慢，因此可以
...
基本块：只有一个前驱和一个后继的流图节点不是很有趣

- 可以将它们与它们的前任和后继合并
- 获得具有较少节点的图，其中每个节点代表一个基本块
...
大小为 $N$ 的程序：最多 $N$ 个节点和最多 $N$ 个变量

- 每个集合并集操作都需要 $O(N)$ 时间
- For 循环：计算每个节点具有恒定数量的集合操作，即有 $O(N)$ 个节点 => $O(N^2)$ 时间
...
- 如果在某些执行中节点 n 之后需要 a，我们可以确保在方程的任何解中都有 $a \in out[n]$
...
- Static liveness (over-approximation，保守，一定包含所有活跃变量)
    - 如果存在从 n 到 a 的 use 的某个节点，且不经过 a 的 def 的某条控制流边路径，则变量 a 在节点 n 处 staticall live
- Dynamic liveness (under-approximation，不一定包含所有活跃变量)
    - 如果程序的某些执行从 n 转到 a 的 use 而不经过 a 的任何 def，则变量 a 在节点 n 处 dynamic live
